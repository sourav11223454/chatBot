...
import '../StickmanLogout/StickmanLogout.css';
import { useLogoutAnimation } from '../StickmanLogout/useLogoutAnimation';

interface StickmanLogoutProps {
  onLogout?: () => void;
  buttonText?: string;
  className?: string;
  variant?: 'light' | 'dark';
...
const ANIMATION_STATES: Record<AnimationState, Record<string, string>> = {
  default: {
    '--figure-duration': '100',
...
export const StickmanLogout: React.FC<StickmanLogoutProps> = ({
  ...
  const updateAnimationState = (state: AnimationState) => {
    ...
    if (buttonRef.current) {
      for (const [property, value] of Object.entries(ANIMATION_STATES[state])) {
        buttonRef.current.style.setProperty(property, value);
      }
    }
  };
  ...
  const handleClick = () => {
    if (animationState === 'default' || animationState === 'hover') {
      ...
      updateAnimationState('walking1');
      
      setTimeout(() => {
        setButtonClasses('clicked door-slammed');
        updateAnimationState('walking2');
        
        setTimeout(() => {
          setButtonClasses('clicked door-slammed falling');
          updateAnimationState('falling1');
          
          setTimeout(() => {
            updateAnimationState('falling2');
            
            setTimeout(() => {
              updateAnimationState('falling3');
            ...
            }, parseInt(ANIMATION_STATES.falling2['--walking-duration']));
          }, parseInt(ANIMATION_STATES.falling1['--walking-duration']));
        }, parseInt(ANIMATION_STATES.walking2['--figure-duration']));
      }, parseInt(ANIMATION_STATES.walking1['--figure-duration']));
    }
  };

  return (
    <div className={`background background--${variant} ${className}`}>
      <button
        ref={buttonRef}
        className={`logoutButton logoutButton--${variant} ${buttonClasses}`}
        onMouseEnter={handleMouseEnter}
        onMouseLeave={handleMouseLeave}
        onClick={handleClick}
        aria-label="Log out"
        disabled={animationState !== 'default' && animationState !== 'hover'}
      >
